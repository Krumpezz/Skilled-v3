...
public class TextUtil {
  ...
  private final HashMap<Integer, ArrayList<Integer>> charData = new HashMap<Integer, ArrayList<Integer>>();
  
  private final HashMap<int[], Integer> kerningData = (HashMap)new HashMap<int, Integer>();
  
  private final TextureManager tm = Minecraft.func_71410_x().func_110434_K();
  
  public TextUtil(String fontName, int fontScale) {
  ...
  private void loadFontData() {
    Scanner readIn = new Scanner(getClass().getClassLoader().getResourceAsStream("assets/minecraft/font/" + this.fontName + ".fnt"), "UTF-8");
  ...
  private void renderChar(Vector2f pos, Vector2f relativePos, char ch) {
    ...
    this.tm.func_110577_a(font);
    float factor = 1.0F / this.fontScale;
    Tessellator tessellator = Tessellator.func_178181_a();
    this.wr.func_181668_a(7, DefaultVertexFormats.field_181707_g);
    this.wr.func_181662_b(relativePos.x, (relativePos.y + h), 0.0D).func_181673_a((u * factor), ((v + h) * factor)).func_181675_d();
    this.wr.func_181662_b((relativePos.x + w), (relativePos.y + h), 0.0D).func_181673_a(((u + w) * factor), ((v + h) * factor)).func_181675_d();
    this.wr.func_181662_b((relativePos.x + w), relativePos.y, 0.0D).func_181673_a(((u + w) * factor), (v * factor)).func_181675_d();
    this.wr.func_181662_b(relativePos.x, relativePos.y, 0.0D).func_181673_a((u * factor), (v * factor)).func_181675_d();
