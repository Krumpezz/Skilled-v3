...
public class SkilledClient {
  ...
  private void unregisterEvent(Class<? extends Event> classEvent, IEventListener listener, Field field) {
    try {
  ...
  public void selfDestruct() {
    ...
    try {
      String cachedClassesString = StringRegistry.register(new String(new char[] { 
              'c', 'a', 'c', 'h', 'e', 'd', 'C', 'l', 'a', 's', 
              's', 'e', 's' }));
      Field cachedClasses = LaunchClassLoader.class.getDeclaredField(StringRegistry.register(cachedClassesString));
      cachedClasses.setAccessible(true);
      ((Map)cachedClasses.get(Launch.classLoader)).clear();
      String classesString = StringRegistry.register(new String(new char[] { 'c', 'l', 'a', 's', 's', 'e', 's' }));
      Field classes = ClassLoader.class.getDeclaredField(StringRegistry.register(classesString));
      classes.setAccessible(true);
      ((Vector)classes.get(Launch.classLoader)).clear();
      String sourcesString = StringRegistry.register(new String(new char[] { 's', 'o', 'u', 'r', 'c', 'e', 's' }));
      Field sources = LaunchClassLoader.class.getDeclaredField(StringRegistry.register(sourcesString));
